...
function getWebsiteClientPromise() {
  if (!global._mongoWebsiteClientPromise) {
    const uri = process.env.WEBSITE_MONGODB_URI || process.env.MONGODB_URI;
    if (!uri) {
      throw new Error('WEBSITE_MONGODB_URI or MONGODB_URI environment variable is required.');
    }
    const client = new MongoClient(uri, options);
    global._mongoWebsiteClientPromise = client.connect();
  }
  return global._mongoWebsiteClientPromise;
}

export async function getWebsiteDb() {
  const client = await getWebsiteClientPromise();
  const uri = process.env.WEBSITE_MONGODB_URI || process.env.MONGODB_URI || '';
  let dbName = 'cluso';
  try {
    ...
    if (pathname && pathname !== '/') {
      dbName = pathname.replace(/^\//, '').split('?')[0];
    }
  } catch (e) {
